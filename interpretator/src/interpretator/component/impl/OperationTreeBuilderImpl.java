...
public class OperationTreeBuilderImpl implements OperationTreeBuilder {
	...
	private Operation readDoWhile(SourceLine sourceLine, String[] tokens, ListIterator<SourceLine> iterator) {
        ...
        checkDoWhileSyntax(doWhileTokens);
        String[] conditionalExpression = Arrays.copyOfRange(doWhileTokens, 2, doWhileTokens.length - 1);
		return new DoWhileOperation(tokens, sourceLine, trueOperations, conditionalExpression);
	}

	private void checkDoWhileSyntax(String[] doWhileTokens) {
        if (!"while".equals(doWhileTokens[0])) {
            throw new SyntaxInterpretatorException("Invalid syntax for do while");
        }
		if(doWhileTokens.length < 4 || !"(".equals(doWhileTokens[1]) || !")".equals(doWhileTokens[doWhileTokens.length - 1])) {
            throw new SyntaxInterpretatorException("Invalid syntax for do while");
        }
	}	
